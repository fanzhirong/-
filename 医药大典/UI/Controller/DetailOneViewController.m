...
-(void)setTableView
{
    ...
    self.table.header = [MJRefreshNormalHeader headerWithRefreshingTarget:self refreshingAction:@selector(headFresh)];
    
    [self.table.header beginRefreshing];
    
    
    self.table.footer = [MJRefreshAutoNormalFooter footerWithRefreshingTarget:self refreshingAction:@selector(footFresh)];
...
-(void)getNetData
{
    if (self.page == 0)
    {
    [NetWorking getDetailFirstDataWithPath:DetailPath page:self.page back:^(NSArray *arr) {
